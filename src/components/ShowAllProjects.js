...
export default function ShowAllProjects({ projects,openProject }) {
        ...
        if (projects.length > 0) {
            return (
                <div>
                    ...
                    {tempProjects.length>0?tempProjects.map(x => <SubCard openProject={openProject} project={x} />):projects.map(x => <SubCard openProject={openProject} project={x} />)}
                </div>
            )
        }
        else {
